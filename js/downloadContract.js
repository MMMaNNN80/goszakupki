...
chrome.runtime.onMessage.addListener( (action,sender) => {
        if (action.message === START) {
            x.message = START
            x.text = `  активна ${activePage? activePage : 1} вкладка  из ${endPage}`
            sendMessages(x)
        ...
        else if (action.message === DOWNLOADCONTRACT)  { downLoadContract() }
